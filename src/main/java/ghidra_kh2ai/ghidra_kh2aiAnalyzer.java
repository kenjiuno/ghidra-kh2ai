...
public class ghidra_kh2aiAnalyzer extends AbstractAnalyzer {
	...
	private static final String name = "Function pointers resolver";
	private static final String desc = "This analyzer scans KH2 AI files for function pointers pushed as values and resolves them.";
	public boolean new_pass = false;

	public ghidra_kh2aiAnalyzer() {
		super(name, desc, AnalyzerType.INSTRUCTION_ANALYZER);
	}
	...
	public boolean canAnalyze(Program program) {
		boolean canAnalyze = program.getLanguage().getProcessor()
				.equals(Processor.findOrPossiblyCreateProcessor("kh2ai"));
		if (canAnalyze) {
			return true;
		}
		return false;
	...
	public void getValueLabel(Program program, Instruction instruction, Listing listing) {
		...
		if (val.getValue() != 0) {
			...
			if (createOneByteFunction(program, null, rel, true) == false) {
				new_pass = true;
			}
	...
	public boolean added(Program program, AddressSetView set, TaskMonitor monitor, MessageLog log)
		...
		while (instructionIterator.hasNext()) {
			...
			if (mnemonicString.contains("syscall")) {
				...
				// TODO: store syscalls/args positions and pointers in a resource file and do
				// that automagically
